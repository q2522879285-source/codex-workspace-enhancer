...
右侧 **Skills → 默认执行 → 添加/删除** 进入管理。可搜索或按分类添加，已有默认项显示“删除”；点击“完成”后只显示标签。删除仅移出当前任务默认，不卸载技能，也不影响其他任务或普通收藏。

默认项保存在当前任务摘要的 `agreements` 中，以 `默认执行 · ` 为前缀，包含所选技能的名称及本机 `SKILL.md` 路径。通用版初始为空，不复制作者的默认项。修改不向输入框写文字，不发送消息，不覆盖独立笔记或摘要其他字段。

要让助手自动收到这些设置，需先按下文启用并信任摘要提醒钩子。保存后从下一条消息起提醒助手读取对应 Skill；当前明确要求及更高优先级规则仍优先。仅保存默认项不等于已执行，也不会绕过钩子信任。

默认项是任务状态，应由界面修改；维护摘要时保留用户当前选择，不恢复已删除项。已有的纯文字默认约定也可以在此移除。
