...
try {
  const readActual = () => client.evaluate(`(() => {
      ...
      return usage ? {
        ...
        fillStyleTransform: fill?.style.transform || "",
        fillVisualWidth: fill?.getBoundingClientRect().width || 0,
        trackVisualWidth: fill?.parentElement?.getBoundingClientRect().width || 0,
  ...
  do {
    ...
    const expectedScale = (expected.remainingPercent ?? 0) / 100;
    const fillSettled = actual?.fillStyleTransform === `scaleX(${expectedScale})`;
  ...
  assert.equal(actual.fillStyleTransform, `scaleX(${(expected.remainingPercent ?? 0) / 100})`);
  if (actual.visibilityState === "visible" && expected.remainingPercent != null) {
    assert.ok(Math.abs(actual.fillVisualWidth / actual.trackVisualWidth * 100 - expected.remainingPercent) < 1);
  }
