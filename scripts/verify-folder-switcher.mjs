...
try {
  ...
  assert.equal(await waitFor(client, `document.querySelector('[data-app-action-sidebar-project-label="示例项目丙"]')?.getAttribute('aria-expanded') === 'true'`, 15_000), true,
  ...
  await client.evaluate(`document.querySelector('[data-codex-sidebar-folder-label="示例项目甲"]')?.click()`);
  assert.equal(await waitFor(client, `document.querySelector('[data-app-action-sidebar-project-label="示例项目甲"]')?.getAttribute('aria-expanded') === 'true'`), true);
  actual = await inspect();
  assert.equal(actual.selected, "示例项目甲");
  assert.deepEqual(actual.visiblePanels, ["示例项目甲"]);
  assert.deepEqual(actual.actionLabels, ["示例项目甲 的项目操作", "在 示例项目甲 中开始新聊天"]);
  ...
  assert.equal(await waitFor(client, `document.querySelector('[data-codex-sidebar-folder-tag][aria-pressed="true"]')?.dataset.codexSidebarFolderLabel === '示例项目甲'`), true,
  ...
  assert.equal(await waitFor(client, `document.querySelector('[data-codex-sidebar-folder-tag][aria-pressed="true"]')?.dataset.codexSidebarFolderLabel === '示例项目乙'`), true,
    "full-catalog search must select the folder assigned in Codex global state");
  assert.equal(await waitFor(client, `(() => {
    const row = document.querySelector('[data-app-action-sidebar-thread-title="整理项目资料"][data-codex-sidebar-search-match="true"]');
  ...
  actual = await inspect();
  assert.equal(actual.selected, "示例项目乙");
  ...
  assert.equal(await waitFor(client, `document.querySelector('[data-codex-sidebar-folder-tag][aria-pressed="true"]')?.dataset.codexSidebarFolderLabel === '示例项目丙'`), true,
    "search must find a project that belonged to a natively collapsed folder");
  actual = await inspect();
  assert.equal(actual.selected, "示例项目丙");
  assert.deepEqual(actual.visiblePanels, ["示例项目丙"]);
