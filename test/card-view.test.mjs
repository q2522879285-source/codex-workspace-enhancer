...
test("missing or invalid saved state defaults to card view", async () => {
  const { normalizeViewMode } = await cardViewModule();
  assert.equal(normalizeViewMode?.(null), "card");
  assert.equal(normalizeViewMode?.("unknown"), "card");
  ...
  const fixtures = [
    {
      preview: {
        title: "示例系列序章",
        summary: "已完成优化并上传交付平台，sendStatus=SUCCESS。",
      },
      want: ["系列项目", "内容优化", "交付完成"],
    },
    {
      preview: {
        title: "安装这个Skills",
        summary: "已完成产品 V2 优化设计，重点修正外壳比例与工业样机感。",
      },
      want: ["产品设计", "Skill安装", "产品优化"],
    },
    {
      preview: {
...
test("examples mentioned in a summary do not override the conversation title scope", async () => {
  ...
  assert.deepEqual(extractPreviewTags?.({
    title: "优化 Codex 对话展示交互样式",
    summary: "标签示例包括人物情绪、Skill开发和交付完成，当前继续调整卡片布局。",
  }), ["Codex侧栏", "界面优化", "卡片视图"]);
});
