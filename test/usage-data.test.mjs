...
test("rate-limit timestamp wins even when log lines arrive out of order", async () => {
  const { parseRateLimitSnapshot } = await usageModule();
  ...
  const snapshot = parseRateLimitSnapshot?.([
    rateEvent("2026-08-11T03:50:00Z", 2),
    rateEvent("2026-08-11T03:40:00Z", 55),
  ]);

  assert.equal(snapshot?.observedAtMs, Date.parse("2026-08-11T03:50:00Z"));
  assert.equal(snapshot?.usage.remainingPercent, 98);
});
