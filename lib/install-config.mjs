...
export function assetBrowserRuntime({ installDir, stateDir } = {}) {
  const sourceRoot = path.join(installDir || path.resolve(import.meta.dirname, '..'), 'asset-browser');
  const stateRoot = path.join(stateDir || process.env.CODEX_ENHANCER_STATE_DIR || (process.platform === 'win32'
    ? path.join(process.env.LOCALAPPDATA || os.homedir(), 'CodexSidebarEnhancer')
    : path.join(os.homedir(), '.local', 'state', 'codex-sidebar-enhancer')), 'asset-browser');
  const files = {
    ASSET_BROWSER_CONFIG: 'asset-browser.config.json', ASSET_BROWSER_TOKEN_FILE: '.api-token',
    ASSET_BROWSER_LEDGER: '.asset-download-ledger.json', GENERATION_TICKETS: '.generation-tickets.json',
    GENERATION_THREAD_BINDINGS: '.thread-project-bindings.json', DUPLICATE_CLEANUP_LEDGER: '.duplicate-cleanup-ledger.json',
    DUPLICATE_QUARANTINE: 'duplicate-quarantine', RHYTHM_CONTROL_REGISTRY: '.rhythm-control-tracks.json',
    PROMPT_LIBRARY_ROOT: 'prompt-library', THREE_D_TASKS: '.three-d-reconstruction-tasks.json',
    ASSET_ACTION_TRASH: 'action-trash', CODEX_WORKSPACE_GOVERNANCE: 'governance',
    MIDJOURNEY_WORKSPACE_REGISTRY: '.midjourney-workspace.json',
  };
  const env = Object.fromEntries(Object.entries(files).map(([key, file]) => [key, process.env[key] || path.join(stateRoot, file)]));
  return { sourceRoot, stateRoot, serverPath: path.join(sourceRoot, 'server.js'), tokenPath: env.ASSET_BROWSER_TOKEN_FILE, env };
}

export async function ensureAssetBrowserState(runtime = assetBrowserRuntime()) {
  await mkdir(runtime.stateRoot, {recursive: true});
  const config = {enabled: true, projects: [], storage: {}, automation: {inbox: {enabled: false, capturePolicy: 'ticketed-only'}, routing: {enabled: false}}, deduplication: {enabled: false}};
  for (const [file, contents] of [[runtime.env.ASSET_BROWSER_CONFIG, JSON.stringify(config, null, 2) + '\n'], [runtime.tokenPath, randomBytes(32).toString('hex')]]) {
    await mkdir(path.dirname(file), {recursive: true});
    await writeFile(file, contents, {flag: 'wx', mode: 0o600}).catch(error => {if (error.code !== 'EEXIST') throw error;});
  }
  return runtime;
}
